...
RUN curl -o apache-drill-1.9.0.tar.gz http://www.eu.apache.org/dist/drill/drill-1.9.0/apache-drill-1.9.0.tar.gz && \
    tar -zxpf apache-drill-1.9.0.tar.gz -C /opt/drill
RUN rm apache-drill-1.9.0.tar.gz

#use DRILL_MAX_DIRECT_MEMORY and DRILL_HEAP to control the memory allocation
#also use  DRILL_CLUSTER and ZOOKEEPER

ADD drill-env.sh /opt/drill/apache-drill-1.9.0/conf/drill-env.sh
